...
class Lua {
	...
	public function run(script:String, ?globals:DynamicAccess<Any>):Any {
		if (globals != null)
			for (key in globals.keys())
				setGlobalVar(key, globals.get(key));
	...
	public function loadLibs(libs:Array<String>) {
		// make openf strictly dynamic, apparently Haxe doesn't denote that (it was null the first time)
		// make the switch case look dumber...

		for (lib in libs) {
			var openf:Dynamic = null;
			...
			switch (lib) {
				...
				case 'os':
					openf = luaopen_os;
				case 'package':
					openf = luaopen_package;
				case 'string':
					openf = luaopen_string;
				case 'table':
					openf = luaopen_table;
				case 'coroutine':
					openf = luaopen_coroutine;
				case _:
					openf = null;
			}
	...
	static function toLuaValue(l, v:Any, ?o:Any):Int {
		switch Type.typeof(v) {
			...
			case t:
				throw 'Cannot convert $t to Lua value';
		}
		return 1;
	}
